    ...
    <>
      <Theme theme="g90">
          ...
          <TabPanels>
            {allLists.map((el) => (
              <TabPanel key={el.list}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 320px)", gridGap: 16 }}>
                  {el?.list &&
                    el?.list.length > 0 &&
                    el?.list.map((item) =>
                      showHiddenValues ? (
                        ...
                        !item?.done && <Task key={item.date} user={user} activeTab={activeTab} {...item} />
                      )
                    )}
                </div>
              </TabPanel>
        ...
        <div style={{ display: "flex", alignItems: "center", marginBottom: 32 }}>
          ...
          <Button kind="danger" onClick={deleteList} style={{ marginRight: 8 }} disabled={allLists.length <= 1}>
            Delete list
          </Button>
      ...
      <AddTask activeTab={activeTab} user={user} isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} />
    </>
  );
};
